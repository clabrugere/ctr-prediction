...
class FeatureSelection(tf.keras.Model):
    ...
    def call(self, embeddings):
        ...
        gate_score_1 = self.gate_1(self.gate_1_bias)  # (batch_size, dim_feature)
        out_1 = 2.0 * tf.nn.sigmoid(gate_score_1) * embeddings

        gate_score_2 = self.gate_2(self.gate_2_bias)  # (batch_size, dim_feature)
        out_2 = 2.0 * tf.nn.sigmoid(gate_score_2) * embeddings
...
class FinalMLP(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        emb_1, emb_2 = self.feature_selection(embeddings)
